...
void cb_connection_got_ip(nm_state_t new_state, int sub_state)
{
	...
	{
		ESP_LOGW(TAG, "IP change detected: old=%d.%d.%d.%d new=%d.%d.%d.%d",
			    IP2STR(&ip), IP2STR(&ipInfo.ip));
		// Only restart if this is not the first IP assignment or if we're not in recovery
		if (!is_recovery_running && ip.addr != 0)
		{
			ESP_LOGW(TAG, "Significant IP change, need to reboot");
			if (!wait_for_commit())
			{
...
{
	// Reset counters if they get too high (safety mechanism)
	if (RebootCounter > 100 || RecoveryRebootCounter > 50)
	{
		ESP_LOGW(TAG, "Resetting reboot counters due to excessive values");
		RebootCounter = 0;
	...
	ESP_LOGD(TAG, "Reboot counters - Total: %d, Recovery: %d", RebootCounter, RecoveryRebootCounter);
...
void app_main()
{
	// Implement boot stabilization early to prevent sporadic boot issues
	implement_boot_stabilization();
	...
	if (bypass_wm == NULL)
	{
		ESP_LOGE(TAG, "Unable to retrieve the Wifi Manager bypass flag, defaulting to false");
		bypass_network_manager = false;
	}
	else
	{
		bypass_network_manager = (strcmp(bypass_wm, "1") == 0 || strcasecmp(bypass_wm, "y") == 0);
		free(bypass_wm); // Free allocated memory
	}
